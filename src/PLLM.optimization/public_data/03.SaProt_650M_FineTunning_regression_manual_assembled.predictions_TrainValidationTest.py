### 20250208
### Yudq
### Description : Using the HuggingFace:westlake-repl/SaProt_650M_PDB model to solve regression problem.
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tuning a regression model using HuggingFace's SaProt_650M_PDB model.")
    parser.add_argument('--seed', type=int, default=42, 
                       help='Random seed for reproducibility, default is 42.')
    parser.add_argument('--csv_path', type=str, required=True, 
                       help='Path to the CSV file containing the dataset.')
    parser.add_argument('--foldseek_path', type=str, required=True, 
                       help='Path to the foldseek.')
    parser.add_argument('--foldseek_util_path', type=str, required=True, 
                       help='Path to the github:SaProt foldseek_util.py file, absolute path better.')
    parser.add_argument('--pdb_path', type=str, required=True, 
                       help='Path to the PDB or CIF file.')
    parser.add_argument('--chain_id', type=str, default="A", 
                       help='Chain ID for the sequence, default is A.')
...
class CustomDataset(Dataset):
    ...
    def load_dataset(self, Class_ID):
        ...
        batch_tokens = self.tokenizer([j for j in df['input']], return_tensors='pt', padding='longest') #Note the difference with ESM2
...
class ESM2Regressor(nn.Module):
    def __init__(self, config):
        ...
        if self.method == 'lora':
            ...
            self.esm2 = EsmModel.from_pretrained(config['model_name'])
            # Freeze all parameters
            for param in self.esm2.parameters():
                param.requires_grad = False
            # Unfreeze top layers
            for param in self.esm2.encoder.layer[-config['top_layers']:].parameters():
                param.requires_grad = True
            # Unfreeze pooler parameters
            for param in self.esm2.pooler.parameters():
                param.requires_grad = True
        else:
            self.esm2 = EsmModel.from_pretrained(config['model_name'])
...
def train_and_evaluate(model, train_loader, val_loader, test_loader, optimizer, scheduler, criterion, num_epochs, logger, device):
    for epoch in range(num_epochs):
        ...
        with torch.no_grad():
            ...
            logger.info(f'{phase} loss: {val_loss:.3f}')
    ...
    # Test phase
    model.eval()
    loader, phase = test_loader, 'Test'
    if loader is None:
        logger.warning("Test loader is None. Skipping test phase.")
        return
    predictions = []
    targets = []
...
def main():
    ...
    data = pd.read_csv(args.csv_path, 
                      names=['mutant', 'labels', 'input', 'Class'], 
                      skiprows=1, index_col=0)
    mean_value     = np.mean(data['labels'])
    std_value      = np.std(data['labels'])
    data['labels'] = (data['labels'] - mean_value) / std_value

    tokenizer = AutoTokenizer.from_pretrained(args.model_name)  

    #Import the get_struc_seq function
    try:
        foldseek_util_dir = os.path.dirname(args.foldseek_util_path)
        sys.path.append(foldseek_util_dir)
        from foldseek_util import get_struc_seq
    except ImportError:
        logger.error("foldseek_util module not found.")
        raise ImportError("Failed to import get_struc_seq from foldseek_util module")

    # Updating input sequence with FoldSeek structure
    seq, foldseek_seq, combined_seq = get_struc_seq( args.foldseek_path, args.pdb_path, plddt_mask=True)[ args.chain_id ]
    data['input'] = data.apply( lambda x: "".join(a + b for a, b in zip(x['input'], foldseek_seq.lower())), axis = 1 )
    ...
    test_dataset = CustomDataset(data, tokenizer, "test")
